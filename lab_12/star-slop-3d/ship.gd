...
func _take_damage(amount: int) -> void:
	if is_invincible:
		print("Damage PARRIED | current HP: " , hp)
		return
	SignalBus.player_damaged.emit()
	hp -= amount
	print("Damage received | current HP: " , hp)
...
func _ready() -> void:
	SignalBus.bullet_dead.connect(_clear_bullet)
	self.add_to_group("player")
	
	$Area3D.body_entered.connect(func(body): _take_damage(1))
